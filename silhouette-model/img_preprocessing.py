...
def squarify_image(img: Image.Image) -> Image.Image:
    ...
    square_image.paste(img, ((target_size - img.width) //
                       2, (target_size - img.height) // 2))
    return square_image
...
# # Testing Data
# input_folder = os.path.join(os.getcwd(), "shoe-model\\test")
# output_folder = os.path.join(os.getcwd(), "silhouette-model\\test")

# # Validation Data
# input_folder = os.path.join(os.getcwd(), "shoe-model\\val")
# output_folder = os.path.join(os.getcwd(), "silhouette-model\\val")
...
for subfolder in os.listdir(input_folder):
    ...
    for filename in os.listdir(subfolder_dir):
        print(filename)
        webp_image = Image.open(os.path.join(
            subfolder_dir, filename)).convert("RGBA")
        output_filename = filename.split('.')[0] + '.jpg'

        # Create a new image with the same size and white background
        white_background = Image.new(
            'RGBA', webp_image.size, (255, 255, 255, 255))
        ...
        squared_image.save(os.path.join(
            output_subfolder_dir, output_filename), 'JPEG')
